# Functions to get a heat map from the protein expression matrix
...
#' Create human proteome heatmap
#' @param full_proteome - all protein expression data, matrix where row names correspond to proteins/gene symbols and columns correspond to sample info
#' @param signatures - list of genes to subset
#' @param title - name of heat map and filename id
#' @param celltypes - vector with (B, ML, LP, S cell types) according to order of full proteome
#' @param average - logical. If true, will map the average expression values across replicates for each cell type
...
# Helper functions
# Get indices of signatures in gene_column when some genes are delimited by ;. eg. when "A" is in signatures, search gene column "A;A2;A3;" which return "A"
...
# Subset proteome by gene list function
