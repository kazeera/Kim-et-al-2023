### ssGSEA Metabric ###
# This script runs an ssGSEA that correlates signatures (gene sets) with Breast Cancer (BC) subtypes (Pam-50 + Claudin Low). RNA expression data is from the METABRIC study from RData file in the folder. 
# adopted from Luis' scripts - modified so that relevant code in 3 scripts compiled into 1 
...
library(foreach)
library(ggfortify)
# PART 1 Create ssGSEAs (ie. Find Enrichment of Signatures in BC expression profiles) -----------------------------
## (1) Creating ssGSEAs
...
## Filtering and cleaning interested data
#cleaning rnaseq.RData object
