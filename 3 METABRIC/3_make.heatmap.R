#' Purpose: 
#' To obtain heat maps from the ssGSEA subtype matrix with PAM50 annotation bar

# Import libraries
library(pheatmap) # heatmap
library(RColorBrewer) # colors gradient
...
#' 
#' HARDCODED arguments
#' @param exp_matrix -rows = name of cell type, columns = ssGSEA correlation
#' @param signatures - list of genes to subset
#' @param title - name of heat map and filename id
#' @param subpopulations - vector with (B, ML, LP, S cell types) according to order of full proteome
make_ssGSEA_heatmap <- function(){
  ...
  # Define title and filename if desired
  ...
  # Finally, create heatmap and save to file
  pheatmap(exp_matrix, 
