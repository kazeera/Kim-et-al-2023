...
compute_ANOVA_tukey_pvals <- function(exp_matrix){
  ...
  # cell_indices <- list(BC = 1:9, LM = 10:19, LP = 20:29) 
  cell_indices <- list(BC = grep("BC", colnames(exp_matrix)), 
                       LM = grep("LC", colnames(exp_matrix)), 
  ...
  ANOVA_tukey <- apply(exp_matrix, 1, function(x){
    ...
    df <- data.frame(
      Expression= unlist(c(x[cell_indices$BC], x[cell_indices$LM], x[cell_indices$LP])),
      CellType =factor(rep(c("BC", "LM", "LP"), times=c(length(x[cell_indices$BC]), length(x[cell_indices$LM]), length(x[cell_indices$LP]))))
    )
    ...
    significance_df <- data.frame(ANOVA_pval = anova_pval,
                                  ...
                                  Tukey_pval_LM.BC = unname(tukey_pvals[grep("LM-BC", names(tukey_pvals))]),
                                  Tukey_pval_LP.LM = unname(tukey_pvals[grep("LP-LM", names(tukey_pvals))])
    )
  ...
  #colnames "Gene, ANOVA_pval, Tukey_pval_LP.BC, Tukey_pval_LM.BC, Tukey_pval_LM.LP"
  return(significance_df)
}
...
compute_logFC_3groups <- function(exp_matrix_avg){
  ...
  log2FC_df2 <- data.frame(log2FC_BC.vs.LM = exp_matrix_avg$BC - exp_matrix_avg$LM, 
                           log2FC_BC.vs.LP = exp_matrix_avg$BC - exp_matrix_avg$LP, 
                           log2FC_LP.vs.BC = exp_matrix_avg$LP - exp_matrix_avg$BC, 
                           log2FC_LP.vs.LM = exp_matrix_avg$LP - exp_matrix_avg$LM, 
                           log2FC_LM.vs.BC = exp_matrix_avg$LM - exp_matrix_avg$BC, 
                           log2FC_LM.vs.LP = exp_matrix_avg$LM - exp_matrix_avg$LP
...
# It returns the original table with columns for whether p-val and FC thresholds met
